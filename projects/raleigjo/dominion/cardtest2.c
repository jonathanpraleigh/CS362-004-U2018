...
int main()
{
	...
	*/
	...
	if (state.handCount[currentPlayer] != originalState.handCount[currentPlayer] + 2) failIfZero = 0;
	if (state.hand[currentPlayer][state.handCount[currentPlayer] - 1] > 6 || state.hand[currentPlayer][state.handCount[currentPlayer] - 1] < 4) failIfZero = 0;
	if (state.hand[currentPlayer][state.handCount[currentPlayer] - 2] > 6 || state.hand[currentPlayer][state.handCount[currentPlayer] - 2] < 4) failIfZero = 0;
	if (state.handCount[currentPlayer] + state.deckCount[currentPlayer] + state.discardCount[currentPlayer] != originalState.handCount[currentPlayer] + originalState.deckCount[currentPlayer] + originalState.discardCount[currentPlayer]) failIfZero = 0;
