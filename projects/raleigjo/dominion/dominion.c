...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case baron:
	baronEffect(choice1, currentPlayer, state);
	return 0;
    ...
    case salvager:
	salvagerEffect(state, choice1, currentPlayer, handPos);
	return 0;
		
    case sea_hag:
	sea_hagEffect(state, currentPlayer);
	return 0;
...
{
	int f_drawntreasure = 0;
	int f_cardDrawn;
	int z=0;
	int f_temphand[MAX_HAND];
	
      	while(f_drawntreasure<2){
		if (state->deckCount[currentPlayer] <1){//if the deck is empty we need to shuffle discard and add to deck
		  shuffle(currentPlayer, state);
		}
	drawCard(currentPlayer, state);
	f_cardDrawn = state->hand[currentPlayer][state->handCount[currentPlayer]-1];//top card of hand is most recently drawn card.
	if (f_cardDrawn == copper || f_cardDrawn == silver || f_cardDrawn == gold)
	  f_drawntreasure++;
	else{
	  z++;
	  f_temphand[z]=f_cardDrawn;
	  state->handCount[currentPlayer]--; //this should just remove the top card (the most recently drawn one).
	}
      }
      while(z-1>=0){
	state->discard[currentPlayer][state->discardCount[currentPlayer]++]=f_temphand[z-1]; // discard all cards in play that have been drawn
	z=z-1;
      }
}
...
void baronEffect(int choice1, int currentPlayer, struct gameState *state)
{
...
void salvagerEffect(struct gameState *state, int choice1, int currentPlayer, int handPos)
{
      //+1 buy
      state->numBuys++;
